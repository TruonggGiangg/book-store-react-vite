  ...
  useEffect(() => {
    const fetchRole = async () => {
      ...
      try {
        ...
        // Cập nhật state
        setCategory(
          responses.filter((item): item is IGetCategories => item !== null)
        );
      } catch (error) {
        ...
        >
          <Descriptions title="Thông tin sách" bordered column={2}>
            ...
            <Descriptions.Item label="Tên sách">
              {dataDetailModal?.title}
            </Descriptions.Item>
            <Descriptions.Item label="Tác giả">
              {dataDetailModal?.author}
            </Descriptions.Item>
            <Descriptions.Item label="Giá bán">
              {dataDetailModal?.price}
            </Descriptions.Item>
            <Descriptions.Item label="Đã bán">
              {dataDetailModal?.sold}
            </Descriptions.Item>
            <Descriptions.Item label="Số lượng tồn">
              {dataDetailModal?.stock}
            </Descriptions.Item>

            <Descriptions.Item label="Đánh giá">
              {dataDetailModal?.rating}
            </Descriptions.Item>
            ...
            <Descriptions.Item label="Ảnh bìa">
              {Array.isArray(dataDetailModal?.coverImage) &&
              dataDetailModal.coverImage.length > 0
            ...
            {dataDetailModal?.isBook === true ? (
              <>
                <Descriptions.Item label="Ngôn ngữ">
                  {dataDetailModal?.attributes?.language}
                </Descriptions.Item>
                <Descriptions.Item label="ISBN">
                  {dataDetailModal?.attributes?.isbn}
                </Descriptions.Item>
                <Descriptions.Item label="Số trang">
                  {dataDetailModal?.attributes?.pages}
                </Descriptions.Item>
                <Descriptions.Item label="Nhà xuất bản">
                  {dataDetailModal?.attributes?.publisher}
                </Descriptions.Item>
                <Descriptions.Item label="Ngày xuất bản">
                  {dayjs(dataDetailModal?.attributes?.publishedDate).format(
                    "YYYY-MM-DD"
                  )}
                </Descriptions.Item>
                <>
