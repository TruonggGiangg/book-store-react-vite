...
const AddBook = (props: IProps) => {
  const { isOpenCreateModal, setIsOpenCreateModal, reload, isBook } = props;
    ...
    const product: ICreateBook = {
      ...
      logo:
        Array.isArray(values.logo) && values.logo.length > 0
          ...
          : "",
      attributes: {
        publisher: values.attributes?.publisher
          ? String(values.attributes.publisher)
          : undefined,
        publishedDate: values.attributes?.publishedDate
          ? new Date(values.attributes.publishedDate)
          : undefined,
        isbn: values.attributes?.isbn
          ? String(values.attributes.isbn)
          : undefined,
        language: values.attributes?.language
          ? String(values.attributes.language)
          : undefined,
        pages: values.attributes?.pages
          ? Number(values.attributes.pages)
          : undefined,
    ...
    try {
      ...
      if (res && res.data) {
        const uploadedFile: any = {
          ...
          status: "done",
          url: `${import.meta.env.VITE_BACKEND_URL}/images/product/${
            res.data.fileName
          }`,
        };
  ...
  return (
    <>
      ...
      <Drawer
        ...
        width={"80%"}
      >
        <Form {...layout} form={form} name="book-form" onFinish={onFinish}>
          <Form.Item
            label="Tựa đề"
            name="title"
            ...
            <>
              <Form.Item label="Thuộc tính sản phẩm">
                <Card>
                  ...
                  >
                    <Input placeholder="Nhập số trang" type="number" />
                  </Form.Item>
